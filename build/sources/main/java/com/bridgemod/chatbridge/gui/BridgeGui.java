...
public class BridgeGui extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList.add(bridgeToggleButton);
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == 1) { // Кнопка переключения
            bridgeToggleButton.toggle();
        } else if (button.id == 2) { // Кнопка "Готово"
    ...
    private class BridgeToggleButton extends GuiButton {
        ...
        private void updateDisplayString() {
            this.displayString = "Чат Бридж: " + (this.enabled ? "ВКЛ" : "ВЫКЛ");
        }
